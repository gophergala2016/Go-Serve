...
func (s listController) Index(rw http.ResponseWriter, req *http.Request) {
	...
	for get_service_list.Next() {
		...
		get_user_details, err := db.Query("SELECT name, image from users where id = $1", User_id)
		if err != nil || get_user_details == nil {
			log.Fatal(err)
		}
		for get_user_details.Next() {
			err := get_user_details.Scan(&Name, &Image)
			if err != nil {
				log.Fatal(err)
			}
		}
		l.Service_Details = append(l.Service_Details, models.User_Service{User_id, Name, Image, Type, Description, Experience, Certificate, Address, City, State, Country})
		no_of_issues++
	}
